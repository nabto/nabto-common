...
| Options | Uri-Path (server routing with `{parameter}` segments), Content-Format, Block1, Block2 and Observe. Uri-Host and Uri-Port are accepted and ignored. Any other critical option is answered with 4.02 Bad Option for a CON request and a RST for a NON request (section 5.4.1). Unknown elective options are ignored. A Block1 or Block2 option that occurs twice is answered the same way as an unknown critical option, per section 5.4.5 and RFC 7959 section 2.1; the client rejects such a response and resets it. Observe and Content-Format are elective, so a repeat of those is ignored rather than answered. |
